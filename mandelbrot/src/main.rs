use num::complex::Complex; //1. importing the Complex number type from the num crate and its complex sub-module
...
fn calculate_mandelbrot( //2. This function converts between the output space (a grid of rows and columns) and a range that surrounds the Mandelbrot set (a continuous region near (0,0))
    ...
    x_min: f64,//4.
    x_max: f64,//4.These four parameters specify the space we’re searching for to look for members of the set
    y_min: f64,//4.
    y_max: f64,//4.
    ...
    for img_y in 0..height { //7. Iterating row by row allows us to print the output line by line
        ...
        for img_x in 0..width {
            let cx = x_min + (x_max - x_min) * (img_x as f64 / width as f64); //8.Calculate the proportion of the space we have covered in our output and convert that to points within the search space
            let cy = y_min + (y_max - y_min) * (img_y as f64 / height as f64); //8.Calculate the proportion of the space we have covered in our output and convert that to points within the search space
